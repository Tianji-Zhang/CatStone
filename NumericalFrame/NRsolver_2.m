    ...
    while max_dx(dvar_table,var_table) >= tolerance
        ...
        for i_S = 1:1:length(S_info)
            for i_eqn = 1:1:length(S_info.S_eqn)
                ...
                R_table(var_1D_index(cell_index,i_eqn)) = R_values - S_values;
                ...
                for i_var = 1:1:length(S_info.dSdv_i);
                    ...
                    Jacob_mat(Jacob_1D_index_mat(cell_index, i_dim, i_var, i_eqn,adj_table)) = Jacob_values -  dSdv_i_values;
                end   
            end
        end
                ...
                for i_eqn = eqn_index_list
                    ...
                    for i_var = var_index_list                                  
                        ...
                        if min(adj_index(cell_index,i_dim,adj_table))>0
                            Jacob_values = Jacob_mat(Jacob_1D_index_mat(cell_index, i_dim, i_var, i_eqn,adj_table));
                            J_values = J_info(i_J). dJdv_adj{i_eqn,i_var}(cell_index, var_table,i_dim,adj_index,adj_table);
                            Jacob_mat(Jacob_1D_index_mat(cell_index, i_dim, i_var, i_eqn,adj_table)) = J_values + Jacob_values;
                        end
                        % i of center cell
                        Jacob_values = Jacob_mat(Jacob_1D_index_mat(cell_index, n_dim*2+1, i_var, i_eqn,adj_table));
                        J_values = J_info(i_J). dJdv_i{i_eqn,i_var}(cell_index, var_table,i_dim,adj_index,adj_table);
