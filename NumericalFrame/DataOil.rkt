...
; must be defined with i since it has no bound option
(define Jzone_2
  (list `[W 0]))
(define BC_eqn1_1 0)
(define BC_eqn2_1 '(/ (- 1000 (p_w i)) dx))

(define Jzone_3
  (list `[E  99]))
(define BC_eqn1_2 '(* (* (kr_o i) k) (/ (- (p_o i) 0) dx)))
(define BC_eqn2_2 '(* (* (kr_w i) k) (/ (- (p_w i) 0) dx)))
