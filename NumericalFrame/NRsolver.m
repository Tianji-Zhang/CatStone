...
Jacob_1D_adj_index = @(cell_index,dim_i)(n_eqn*n_var*n_cell)*(adj_index(cell_index,dim_i)'-1);
        ...
        for i_S = 1:1:length(S_info)
            for i_eqn = 1:1:length(S_info.S_eqn)
                ...
                for i_var = 1:1:length(S_info.dSdv_i);
                    Jacob_values = Jacob_mat(Jacob_1D_index_mat(cell_index, i_dim, i_var, i_eqn));
                    dSdv_i_values = S_info(i_S). dSdv_i{i_eqn,i_var}(cell_index,var_table);
                    Jacob_mat(Jacob_1D_index_mat(cell_index, i_dim, i_var, i_eqn)) = dSdv_i_values + Jacob_values;
                end
        ...
        for i_J = 1:1:length(J_info)
            for i_dim = 1:n_dim*2
                for i_eqn = eqn_index_list
                    ...
                    R_values = R_table(var_1D_index(cell_index,i_eqn));
                    J_values = J_info(i_J). J_eqn{i_eqn}(cell_index,var_table,i_dim);
                    ...
                    for i_var = var_index_list
                        ...
                        % boundary?
                        % need to be improved
                        if min(adj_index(cell_index,i_dim))>0
                            Jacob_values = Jacob_mat(Jacob_1D_index_mat(cell_index, i_dim, i_var, i_eqn));
                            J_values = J_info(i_J). dJdv_adj{i_eqn}(cell_index, var_table,i_dim);
                        end
                        % i of center cell
                        Jacob_values = Jacob_mat(Jacob_1D_index_mat(cell_index, n_dim*2+1, i_var, i_eqn));
                        J_values = J_info(i_J). dJdv_i{i_eqn}(cell_index, var_table,i_dim);
                        Jacob_mat(Jacob_1D_index_mat(cell_index, 2*n_dim+1, i_var, i_eqn)) = J_values + Jacob_values;
                    end
